...
class TableSegment {
  ...
  constructor({ knex, databaseConnectionConfig, table, where, minId, maxId, ignoreColumns, releventColumns = [] }) {
    if (knex) {
      this.knex = knex;
    } else {
      this.knex = Knex({
  ...
  async prepareMinMaxId() {
    ...
    const [{ maxId, minId }] = await this.knex(this.table).where(this.where).select(this.knex.raw('max(id) as maxId, min(id) as minId'));
...
async function hyperDiffMerge({
  oldDatabaseConnectionConfig,
  oldTable, oldDataWhere,
...
//   const result = await hyperDiffMerge({
//     oldDatabaseConnectionConfig: {
//       host: '127.0.0.1',
//       port: 3306,
//       user: 'root',
